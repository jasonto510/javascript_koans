...
describe("About Higher Order Functions", function () {
  ...
  it("should use 'forEach' for simple iteration", function () {
    var numbers = [1,2,3];
    var msg = "";
    var isEven = function (item, index, arr) {
      msg += (item % 2) === 0;
    };
  ...
  it("should use range to generate an array", function() {
    expect(_.range(3)).toEqual([0,1,2]); //all the numbers til this point
    expect(_.range(1, 4)).toEqual([1,2,3]);  //all the numbers til this point including teh first number
    expect(_.range(0, -4, -1)).toEqual([0,-1,-2,-3]); //all the numbers til this point iterating by -1
  });
  ...
  it("should use chain() ... .value() to use multiple higher order functions", function() {
    var result = _([ [0, 1], 2 ]).chain()
                     ...
                     .value(); //returns the value of whatever it is
